...
export TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_APPIDS2="${TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_APPIDS_DEFAULT}"
# Notification symbol
export TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_CHAR2="${TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_CHAR_DEFAULT}"
EORC
	echo "${rccontents}"
}

__process_settings() {
	if [ -z "$TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_APPIDS2" ]; then
        export TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_APPIDS2="${TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_APPIDS_DEFAULT}"
	fi
	if [ -z "$TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_CHAR2" ]; then
		export TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_CHAR2="${TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_CHAR_DEFAULT}"
	fi
}
...
run_segment() {
	...
	app_ids_array=(${TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_APPIDS2})
    ...
    query_string="select count(*) from record ${query_condition}"
	...
	if [[ -n "$count"  && "$count" -gt 0 ]]; then
		echo "${TMUX_POWERLINE_SEG_MACOS_NOTIFICATION_COUNT_CHAR2} ${count}"
	fi
